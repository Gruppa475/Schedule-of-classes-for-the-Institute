...
class DPCalendarModelBooking extends JModelAdmin
{
	...
	public function save($data, $invite = false)
	{
		...
		if (!array_key_exists('id', $data) || !$data['id']) {
			...
			if (JFactory::getApplication()->isClient('site') && !$invite) {
				$data['user_id'] = $user->id;
			}

			foreach ((array)$data['event_id'] as $eId => $types) {
				$event           = $this->getEvent($eId);
				$paymentRequired = false;

				$data['options'] = [];
				if (!empty($types['options'])) {
					...
					foreach ($event->booking_options as $key => $option) {
						...
						$data['options'][] = $eId . '-' . $key . '-' . $amount[$key];

						$data['price'] += $amount[$key] * $option->price;
						$data['state'] = 3;
					}
				}
			...
			// Unset the price, that it can't be changed afterwards through some
			// form hacking
			if (!$user->authorise('core.admin', 'com_dpcalendar') && JFactory::getApplication()->isClient('site')) {
				unset($data['price']);
			}
	...
	private function getAmountTickets($event, $data, $amount, $index)
	{
		...
		if ($amountTickets < 1 && $amount[$index] > 0) {
			...
			JFactory::getApplication()->enqueueMessage(
				JText::sprintf('COM_DPCALENDAR_BOOK_ERROR_CAPACITY_EXHAUSTED_USER', $event->price ? $event->price->label[$index] : '', $event->title),
				'warning'
			);
		}
